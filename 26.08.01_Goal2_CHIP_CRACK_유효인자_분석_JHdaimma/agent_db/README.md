...
**현재 분포**(총 78행): 팀문서 24 · **멘토_확정 17** · **작성자_추론 14** · 현업_확정 10 · 멘토_미확정 8 · 데이터_실증 3 · 기타 2

> **26.08.01 갱신**: `Edge_Burn`/`Edge_Burn_Die`가 `멘토_미확정` → `멘토_확정`으로 승격됐습니다.
> 김시우 브랜치(커밋 `7e2d73c`)에서 멘토가 "유효한 실패모드 아님"으로 최종 확인해
> defect 분석 대상에서 제외됐습니다. Jun의 BURN 분석 전체는 무효화되지만, 이 DB는
> `Edge_Burn`을 피처로 쓴 적이 없고 `NG_Code=='BURN'` 608건 중 Chipping·Micro_Crack
> 동시발생이 0건임을 검증해 **영향이 없습니다.**

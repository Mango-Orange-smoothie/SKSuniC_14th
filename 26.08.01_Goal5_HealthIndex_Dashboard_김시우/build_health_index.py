...
다시 짰다.)
  ...
  2. **추세는 두 가지를 별도 정보로 계속 제공하면서, 그중 (b)만 점수에도 작게 반영한다.**
       ...
       (26.08.06: (b)를 "따로 보여주기만" 했더니, margin은 안 닳았는데 CUSUM은 이미
        ...
        0.281(26.08.18 재측정)로 낮아 어느 쪽으로도 대체가 안 되므로 둘 중 큰 쪽을 쓴다.)
...
# 같은 사각지대인데 안전망에는 그 논리가 없었다. 도입 당시 실측: 확정 원인이 아닌데
# 경보가 켜진 컬럼 59건 중 **55건이 아무 데도 안 나왔다** — DP03 Kerf_Angle은 37일째
# 경보인데 margin 18.7%라 빠졌다.
# (26.08.18 재측정: 해당 컬럼 31건 중 안전망에 걸리는 건 4건, 나머지 27건은 여전히
# 아무 데도 안 나온다. 그리고 그 4건은 **전부 지속경보 조건으로만** 걸린다 —
# margin>=50% 조건 하나였으면 31건 중 0건이었다. 이 조건을 넓힌 이유가 그대로 유효하다.)
#
...
# 이 경우: 활성 경보 44건 중 35건이 14일 미만, 지속일 중앙값 1.1일)는 아직 증거가 얕으니
# 레벨을 거의 안 건드리고,
...
# 실제 데이터 180개 장비×컬럼 중 margin이 100%를 넘는 건 0건이라(최댓값 90.8%),
# 여기에 스케일을 많이 떼어줄 이유가 없다. (26.08.18 재측정. 옛 주석의 "140개 중 3건"은
# 분모가 spec_source=control_limit_3sigma인 행 수였고 분자도 라벨 확장 전 값이었다.) 10점이면 스펙 안 영역은 health = 100 - 0.9×margin이 되어
...
def compute_level_and_trend(
    ...
    for (machine, col), g in daily_series.groupby(["Machine_ID", "column"]):
        ...
        # (도입 당시) 활성 경보 76건 중 63건이 세기 0.5 미만이고 그 63건의 HI가 전부 96
        # 이상이라, (26.08.18: 44건 중 35건이 0.5 미만, 그 35건의 HI는 전부 80.7 이상)
        ...
        if alert_strength is not None and not alarm_exceeded:
            ...
            # 순위상관은 0.281로 낮고, 활성 경보 44건 중 19건(43%)은 기울기가

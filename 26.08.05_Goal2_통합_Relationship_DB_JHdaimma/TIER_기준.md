...
`Chipping / Vibration`, `Micro_Crack / Vibration` **2건을 뺐습니다.**

> **유효인자(원인) 트랙이 아니라 별도 알람 트랙으로 운영**하기로 했기 때문입니다.
> **추세 상승**과 **spec 이탈**만 감시해 알람을 주고, **조치 지시는 하지 않습니다.**

`Vibration`은 *"값을 낮추라"*고 지시할 수 있는 인자가 아닙니다. 설비 정비 대상이므로
원인 인자와 같은 표에 두면 Agent가 실행 불가능한 조치를 제안하게 됩니다.

**알람 규칙은 별도 산출물로 관리합니다.** 스크립트의 `VIBRATION_EXCLUDED`에 제외 사유가 기록돼 있습니다.

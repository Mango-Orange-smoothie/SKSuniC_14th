...
- `Particle`, `Remain_Coat`, `Edge_Burn`을 각각 별도 모델로 봅니다.
...
예를 들어 특정 장비의 `Head_Temp_7d_ma`가 상승한 뒤 `Yield_7d_ma`가 하락하거나 `Edge_Burn_rate`가 상승하는지 확인합니다. 반드시 같은 Product/Recipe 조건으로 다시 나누어 재확인해야 합니다.

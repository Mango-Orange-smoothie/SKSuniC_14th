...
| false면 "없음(조짐 단계)"}
- **원인**: `{worst_factors[0]의 factor}` 현재 `{current_value}` / {direction이 up이면 "상한" down이면 "하한"} \
`{usl 또는 lsl}` [{spec_source가 mentor_spec이면 "스펙기준" provisional_percentile이면 "임시기준"}], \
{trend_direction이 up이면 "상승" down이면 "하강" 아니면 생략}추세{estimated_days_to_spec_out이 \
...
헤더 없이 불릿 한 줄로: `- **{N}순위** {defect} (HI {값}) [발생/미발생] — {worst_factor} {한 줄 사유}`)
...
9. 한국어로 답하되, 위 출력 형식을 벗어난 부가 설명 문단을 붙이지 마라. 사용자가 형식에 없는 걸 \

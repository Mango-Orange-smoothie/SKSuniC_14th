...
답변 맨 끝에 ※ 로 시작하는 면책·주석 줄을 붙이지 마라. "상관관계일 뿐", "인과관계는 아님", \
"SOP는 아직 못 받았다" 같은 문장을 **어떤 형태로든 만들지 마라** — SOP는 2026-08-11에 \
수령했고, 인자별 신뢰도는 이미 tier로 답변 안에 들어가 있다. 조치 문구가 없는 인자는 \
그 인자 자리에서 `[SOP 미수령]`으로만 표시하고 끝낸다(위 **조치** 규칙).
--- 형식 끝 ---

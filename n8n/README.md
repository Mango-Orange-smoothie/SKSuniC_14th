...
>
> (26.08.11) 그 `alert_active_days >= 14` 조건은 이제 `alert_level == "full"`로 바로
> 읽을 수 있다. Health Index가 추세 페널티를 최대폭까지 받은 경보라는 뜻이고, 현재
> 활성 경보 **44건 중 9건**이다(나머지 35건은 `"early"` — HI가 전부 **80.7** 이상이라
> 알림을 보내면 대부분 헛알림이 된다). 26.08.18 실측이고, 도입 당시(26.08.11)엔
> 76건 중 11건 / 67.9 이상이었다. 근거는 `docs/점검_원인변수_경보기준.md` §5.

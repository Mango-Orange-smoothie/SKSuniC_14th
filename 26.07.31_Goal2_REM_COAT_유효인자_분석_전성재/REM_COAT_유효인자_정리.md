...
> Jun 브랜치의 1차 산출물(전처리·REM_COAT 유효인자 발굴)에서 출발해, 독립적인 재현·확장
> 검증을 거쳐 최종 결론에 도달한 기록이다. 아래 요약이 결론, 1~14번은 거기까지
> 어떻게 도달했는지의 상세 과정(참고용)이다.

## 요약

1. **REM_COAT(코팅 잔류)의 원인은 `CLN_Pressure`(세정 압력) 하나입니다.** Jun의 1차
   분석부터 daeho의 완전 독립 교차검증, 팀 공용 기존 분석(10번), SHAP(12번)까지
   **6가지 서로 다른 방법**으로 확인했는데 전부 같은 답이 나왔고, 불량률을 3배 높인 새
   데이터로도 흔들리지 않았습니다.
2. **서서히 나빠지는 게 아니라, 그 스트립을 세정하는 순간 압력이 튀는 즉시성 현상입니다**
   (9·11번 검증). 그래서 SOP는 "압력 추세 감시"가 아니라 **"스트립별 실시간 압력 급락
   알람"**으로 설계해야 합니다.
3. **`Coating_Thickness`는 통계적으로는 강하지만 후보에서 제외**했습니다 — 세정 후에 잰
   값이면 잔류 코팅량과 사실상 같은 걸 두 번 재는 셈이라(데이터 누수 위험), 측정 시점을
   현업에 확인하기 전까지는 원인으로 보고하면 안 됩니다.
4. **AI Agent가 바로 쓸 수 있는 형태까지 만들어뒀습니다** — SHAP로 스트립 개별 설명(12번),
   불량 스트립 2,332건 전체의 사전계산 설명 DB(13번)를 만들어서 "이 스트립 왜
   불량났어?"라는 질문에 Agent가 즉시 조회만으로 답할 수 있게 했습니다(김시우님 Agent
   프로토타입 구조에 맞춤, 단 확정 구조는 아님).
5. **(별도 문서) DP04 장비가 유독 REM_COAT가 많은 이유도 확장 분석 완료** —
   `26.08.01_Goal1_장비제품비교_전성재/`에서 CLN_Flow(세정 유량) 만성 저하가 DP04
   초과위험을 103% 설명함을 매개분석으로 확인.
6. **아직 데이터로 못 풀고 사람에게 물어봐야 하는 것 3가지**(월요일 회의 우선순위):
   ① `Coating_Thickness` 측정 시점(가공 전/후) — 멘토님께, 데이터 누수 여부 판가름
   ② `CLN_Pressure`가 스트립마다 실시간으로 변하는 값인지 장비 세팅값인지 — 현업에,
   2번 결론(즉시성 현상)이 맞는지 검증
   ③ DP04의 `CLN_Flow`가 왜 만성적으로 낮은지(노즐 막힘/펌프 노후 등) — 정비팀에,
   Goal1 결과를 실제 조치로 연결하려면 필요

바쁘시면 위 요약만 보셔도 됩니다. 아래는 그 결론에 이르기까지의 상세 과정입니다.
...
## 15. 차별화 아이디어 (미착수, 제안 단계)

지금까지 팀 전체가 "원인이 뭐냐"(진단) 위주였다면, 아래는 "그래서 뭘 하면 되냐"(처방/예측)
쪽으로 한 발 더 간 아이디어들. 아직 아무것도 구현 안 함 — 회의에서 방향 정해지면 착수.

1. **What-if 시뮬레이터**: Goal1 매개분석 모델(오즈비 1.70→0.98)을 역으로 써서
   "CLN_Flow를 얼마나 올리면 예상 불량률이 얼마나 주는지" 계산기
2. **경제적 임팩트 환산**: "DP04만 고치면 연간 예상 절감 스트립 수/비용"으로 발생률 차이를
   비즈니스 언어로 변환
3. **신뢰도 배지**: Agent 답변에 "7가지 방법 중 몇 개 일치"를 같이 표시 — 저희만 가진
   재현성 검증(10·11번)을 답변 신뢰도 지표로 노출
4. **진단서 자동 생성**: 13번(스트립DB) + Goal6 SOP 등급을 조합해서, 불량 스트립 하나
   넣으면 품질 엔지니어가 손으로 쓸 법한 리포트를 자동 생성

## 16. 원인/감시지표/불량결과 분류 (JHdaimma 관계DB 통합 요청 대응)

JHdaimma가 팀 공용 "Relationship DB" 통합을 준비하면서, 각 담당자(Particle=daeho,
Remain_Coat=전성재, Chipping/Crack=JHdaimma) DB에 **원인(조치 가능)/감시지표(관찰만)/
불량결과** 분류를 추가해달라고 요청함 — Agent가 "3일 뒤 OO 발생 가능, XX 조치하세요"
처럼 원인과 결과를 구분해서 답하려면 이 분류가 필수이기 때문.

| 분류 | 항목 | 근거 |
|---|---|---|
| **원인 (조치 가능)** | `CLN_Pressure` | 확정 유효인자(4·5·6·10·12번), 스트립별 실시간 알람 대상 |
| **원인 (조치 가능, 장비 한정)** | `CLN_Flow` | DP04에서만 원인(Goal1 매개분석, 초과위험 103% 설명) — 전체 장비 공통 원인 아님, "DP04 한정"으로 표시 필요 |
| **불량결과** | `Remain_Coat` | defect 그 자체 |
| **분류 보류** | `Coating_Thickness` | 측정 시점(가공 전/후) 미확인 — 원인인지 불량결과와 동어반복(사실상 감시지표도 아닌 "같은 것을 두 번 잰 값")인지조차 아직 불명확. **현업 확인 전까지 어느 분류에도 넣지 말 것** |
| — | `CLN_Time` | 9번에서 원인 후보 완전 제외됨 — 관계DB 통합 시 넣지 않음 |

**감시지표(관찰만) 항목이 REM_COAT엔 없다** — 11번(전체 후보 39개 선행신호 전수조사)에서
확인했듯 REM_COAT는 daeho의 Vibration(PARTICLE)처럼 "서서히 나빠지다가 감시로 미리
잡아내는" 유형의 원인이 아예 없는 defect였기 때문. 이것도 관계DB 통합 시 "REM_COAT는
왜 감시지표가 없는지" 설명이 필요할 수 있음(즉시성 현상이라 사전 감시가 원리적으로
어려움 — 9번 참고).

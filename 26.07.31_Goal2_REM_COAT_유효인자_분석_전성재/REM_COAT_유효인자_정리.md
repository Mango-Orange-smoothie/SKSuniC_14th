...
> 검증을 거쳐 최종 결론에 도달한 기록이다. 아래 세 줄 요약이 결론, 1~12번은 거기까지
> 어떻게 도달했는지의 상세 과정(참고용)이다.

## 세 줄 요약

1. **REM_COAT(코팅 잔류)의 원인은 `CLN_Pressure`(세정 압력) 하나입니다.** Jun의 1차
   분석부터 daeho의 완전 독립 교차검증까지 **5가지 다른 방법**으로 확인했는데 전부 같은
   답이 나왔고, 불량률을 3배 높인 새 데이터로도 흔들리지 않았습니다.
...
바쁘시면 위 세 줄만 보셔도 됩니다. 아래는 그 결론에 이르기까지의 상세 과정입니다.
